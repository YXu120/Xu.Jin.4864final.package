...
From previous running output, the first value is the point estimate of $\beta_1$ after fitting the model. This is the original value of $\beta_1$ that we obtain before doing bootstrap to refit the model. The second group of value is the $\beta_1^*$ we obtain after refitting model with new generated random effects. A clear observation here is, all these $\beta_1^*$ are around the original point estimate, some above it, some below it, but still value close. This phenomenon also can be viewed from very small standard error, which is the third value. It means these data do not cause value of $\beta_1$ fluctuating a lot. The forth part of output is our 95% confidence interval. 
...
Here is the process of picking best value of B, the sample size of bootstrap. We aim to pick a proper value of B and we expect it will converge at some point.
```{r}
std <- data.frame(index = seq(from = 10, to = 300, by = 10)) %>%
  group_by(index) %>%
  summarize(boot_sample = suppressMessages(para_boot(tortoise, "tortoise", index))) %>%
  mutate(value = (boot_sample[[1]] - boot_sample[[2]])^2 / (index-1) )

sd_plot <- data.frame(B = std$index, Standard_deviation = std$std_value) %>%
